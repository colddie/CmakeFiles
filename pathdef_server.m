addpath(genpath('/code/matlab/ENIGMA-2.0.0'))
addpath(genpath('/code/matlab/minepy-1.2.6/matlab'))
addpath(genpath('/code/matlab/magia'))
addpath(genpath('/code/matlab/spm12'))
addpath(genpath('/code/matlab/general'))
addpath(genpath('/code/matlab/viewer3dgui'))
addpath(genpath('/code/matlab/radial2D'))
addpath(genpath('/code/matlab/e7tool_matlab_from_JO'))
addpath(genpath('/code/matlab/pct'))
addpath(genpath('/code/matlab/nifti20140122'))
addpath(genpath('/code/matlab/2019_03_03_BCT'))
addpath(genpath('/code/matlab/gramm'))
addpath(genpath('/code/matlab/BlandAltman'))
addpath(genpath('/code/matlab/DPABI_V6.0_210501'))
addpath(genpath('/code/matlab/BrainNet'))
addpath(genpath('/code/matlab/NBS1.2'))
addpath(genpath('/code/matlab/gift/GroupICATv4.0c/icatb'))
addpath(genpath('/code/matlab/xjview'))
addpath(genpath('/code/matlab/csvimport'))
addpath(genpath('/code/matlab/JuSpace-JuSpace_v1.5'))
addpath(genpath('/data/easystore/data/henan_renmin'))
% addpath(genpath('/code/matlab/WFU_PickAtlas_3.0.5b'))
% addpath(genpath('/code/matlab/REST_V1.8_130615'))
% addpath(genpath('/storage0/home/tsun/code/matlab/PETRECON_MP24_R03_REL_1_28_MRAC_pw-BestToolboxEver01242017'))
% addpath(genpath('/storage3/MLAA/GEtoolbox/PETRECON_MP24_R03_REL_1_28_MRAC_pw-BestToolboxEver01242017/PETRECON_MP24_R03_REL_1_28_MRAC'))
% addpath(genpath('/home/tsun/code/matlab/PETRECON_MP24_R03_REL_1_28_MRAC_pw-BestToolboxEver01242017/PETRECON_MP24_R03_REL_1_28_MRAC/petrecon'))
% addpath(genpath('/home/tsun/code/matlab/Duetto/DUETTO_REL1.1.3/duetto'))
addpath(genpath('/code/matlab/duetto_v02.04/duetto'))
% addpath(genpath('/home/tsun/code/matlab/PETRECON_MP24_R03_REL_1_28_MRAC_pw-BestToolboxEver01242017/PETRECON_MP24_R03_REL_1_28_MRAC/petrecon/unlist'))
% addpath(genpath('/home/tsun/code/matlab/PETRECON_MP24_R03_REL_1_28_MRAC_pw-BestToolboxEver01242017/PETRECON_MP24_R03_REL_1_28_MRAC/petrecon/io_utils'))
addpath(genpath('/code/matlab/nm_signa'))
run('/code/matlab/fessler/irt/setup.m')

diary('matlablog.txt')
datapath = '/data/easystore';

